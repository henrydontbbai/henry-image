...
def test_ci_workflow_validation_requires_python_310_on_all_platforms():
    mod = load_module()
...
def test_ci_workflow_validation_requires_python_310_in_matching_job():
    mod = load_module()
    ...
    assert any(
        "windows job is missing expected CI marker: python-version: [\"3.9\", \"3.10\", \"3.12\"]" in issue
        for issue in issues
    )
